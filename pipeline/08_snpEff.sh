...
if [ ! -f $GFFGENOMEFILE ]; then
	echo "no GFF file $GFFGENOMEFILE"
	exit
fi
if [ -z $SNPEFFJAR ]; then
...
if [ ! -e $SNPEFFOUT/$snpEffConfig ]; then
  ...
  echo "# Cocci " >> $SNPEFFOUT/$snpEffConfig
  echo "$SNPEFFGENOME.genome : Coccidioides immitis WA" >> $SNPEFFOUT/$snpEffConfig
